...
(function () {
  ...
  function mockScorecard() {
    ...
    const total = sum(S.answers), pct = Math.round(total / 75 * 100);
    const weak = {};
  ...
  function explainAsk(query) {
    bubble(query, true);
    const hits = bm25(query, S.grade || 10, S.subject);
    if (!hits.length) return say(T("माफ़ कीजिए, इस विषय पर सामग्री नहीं मिली।", "Sorry, no grounded content found for that."));
    hits.forEach((h) => {
      const cite = h.src.replace(/^NCERT (\w+) ch(\d+)$/, (_, f, ch) => `NCERT ${f} — अध्याय/Chapter ${parseInt(ch)}`);
      say(h.t.slice(0, 380) + "…", "", "📚 स्रोत/Source: " + cite);
    });
  }
